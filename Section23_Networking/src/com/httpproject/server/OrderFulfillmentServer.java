package com.httpproject.server;
...
public class OrderFulfillmentServer {
	...
	public static void main(String[] args) {
		...
		// using 0 or a negative number as the backlog will use system default value
		try {
			HttpServer server = HttpServer.create(new InetSocketAddress(8080), 0);

			server.createContext("/", exchange -> {
				String reqParameters = exchange.getRequestURI().toString();
				System.out.printf("%s %s %s%n", exchange.getRemoteAddress(),
						exchange.getRequestMethod(), reqParameters);
