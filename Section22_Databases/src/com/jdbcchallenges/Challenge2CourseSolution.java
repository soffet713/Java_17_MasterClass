...
public class Challenge2CourseSolution {
	...
	public static void main(String[] args) {
		...
		try (Connection conn = dataSource.getConnection()) {
			...
			DateTimeFormatter dtf = DateTimeFormatter.ofPattern("G yyyy-MM-dd HH:mm:ss") // When using yyyy, need era
					.withResolverStyle(ResolverStyle.STRICT);
			...
			orders.forEach((o) -> {
				...
				try {
					LocalDateTime localDateTime = LocalDateTime.parse("AD " + o.dateString(), dtf); //Add 'AD' era
					...
					cs.execute();
					System.out.printf("%d records inserted for %d (%s)%n", cs.getInt(4),
							cs.getInt(3), o.dateString());
				} catch (Exception e) {
	...
	private static List<Order> readData() {
		...
		try (Scanner sc = new Scanner(Path.of("Orders.csv"))) {
			sc.useDelimiter("[,\\n]");
