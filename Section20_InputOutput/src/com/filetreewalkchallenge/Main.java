...
public class Main {
	...
	public static void main(String[] args) {
		...
		Path path = Path.of(".."); //change path back to current working directory
		FileVisitor<Path> statsVisitor = new StatsVisitor(Integer.MAX_VALUE); //change depth to include all directories
			...
			if (relativeLevel == 1) {
				folderSizes.forEach((key, value) -> {
					...
					if (level < printLevel) {
						...
						System.out.printf("%s[%s] - %,d bytes, %d files, %d folders %n", "\t".repeat(level),
