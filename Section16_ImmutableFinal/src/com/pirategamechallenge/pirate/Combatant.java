...
public sealed abstract class Combatant implements Player permits Islander, Pirate, Soldier {
    ...
    boolean useWeapon(Combatant opponent) {
        ...
        if(new Random().nextBoolean()) {
            ...
            System.out.printf("%s's health = %d, %s's health = %d%n", name, value("health"), opponent.name(),
                    opponent.value("health"));
    ...
    public String information() {
        return name + ": " + gameData + ((getCurrentWeapon()==null) ? "" : " | CurrentWeapon = " + getCurrentWeapon());
    }
}
