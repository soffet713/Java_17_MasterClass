...
public class CouseMainChallenge {
    ...
    public static void main(String[] args) {
        ...
        List<Student> iterateStudents = Stream.iterate(1, s -> s <= 500, s-> s+1)
                .map(s -> Student.getRandomStudent(jmc, pymc))
                .toList();

        List<Student> intStreamStudents = IntStream.rangeClosed(1, 5000)
                .mapToObj(s -> Student.getRandomStudent(jmc, pymc))
                .toList();

        double totalPercent = intStreamStudents.stream()
                .mapToDouble(s -> s.getPercentComplete("JMC"))
                .reduce(0, Double::sum);

        double avePercent = totalPercent / intStreamStudents.size();
        ...
        List<Student> hardWorkers = intStreamStudents.stream()
